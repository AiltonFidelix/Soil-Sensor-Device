...
device = ubinascii.hexlify(machine.unique_id()).decode('utf-8')

ap_ssid = "Soil Sensor - " + device
...
def handle_root(client):
    wlan_sta.active(True)
    ssids = sorted(ssid.decode('utf-8')
                   for ssid, *_ in wlan_sta.scan() if len(ssid) > 0)
    send_header(client)
    client.sendall("""\
            <html>
            <head>
    ...
    client.sendall("""\
            ...
            <label style="margin-top: 18px; color:#277e5f; display: block; text-align: center;">
                By Ailton Fidelix - 2021</label>
            <br>
...
def handle_configure(client, request):
    ...
    if do_connect(ssid, password):
        response = """\
            <html style="background-color: #55ddd5;">
                <head>
                    <meta name="viewport" content="width=device-width, initial-scale=1.0">
                </head>
                <center>
